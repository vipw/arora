...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= QWebSettings::MissingImageGraphic) && (arg <= QWebSettings::TextAreaSizeGripCornerGraphic))
...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= QWebSettings::AutoLoadImages) && (arg <= QWebSettings::LinksIncludedInFocusChain))
...
static QScriptValue qtscript_QWebSettings_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    return qtscript_QWebSettings_throw_ambiguity_error_helper(context,
        qtscript_QWebSettings_function_names[_id+11],
        qtscript_QWebSettings_function_signatures[_id+11]);
}
...
QScriptValue qtscript_create_QWebSettings_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 12; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QWebSettings_prototype_call, function_lengths[i+11]);
        fun.setData(QScriptValue(engine, uint(0xBABE0000 + i)));
        proto.setProperty(QString::fromLatin1(qtscript_QWebSettings_function_names[i+11]),
            fun, QScriptValue::SkipInEnumeration);
    }

    engine->setDefaultPrototype(qMetaTypeId<QWebSettings*>(), proto);

    QScriptValue ctor = engine->newFunction(qtscript_QWebSettings_static_call, proto, function_lengths[0]);
